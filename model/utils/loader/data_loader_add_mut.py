...
import pickle
import numpy as np
from config import BATCH_SIZE, NUM_HOPS, SEED


class EnhancedDataLoader:
    """增强版数据加载器，支持子图提取和突变位点信息"""
    def __init__(self, data_path, batch_size=BATCH_SIZE, val_split=0.1, num_hops=NUM_HOPS):
...
class EnhancedTestDataLoader:
    ...
    def __init__(self, data_path, batch_size=16, num_hops=NUM_HOPS):
        # 之前num_hops=3，现在统一为NUM_HOPS
        self.batch_size = batch_size
        self.num_hops = num_hops
        self.data_list = self.load_and_process_data(data_path)
